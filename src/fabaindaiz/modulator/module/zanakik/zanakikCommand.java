...
import static fabaindaiz.modulator.module.zanakik.zanakikUtil.*;
...
public class zanakikCommand implements CommandExecutor {
    ...
    protected zanakikCommand(Modulator modulator) {

        this.plugin = modulator;
        this.lang = modulator.getConfiguration().getLang();
        this.enabled = plugin.getConfiguration().getConfig().getBoolean("zanakik.enable");
        this.noplayerzkik = plugin.getConfiguration().getConfig().getBoolean("zanakik.noplayerzkik");
        this.ignorebypass = plugin.getConfiguration().getConfig().getBoolean("zanakik.ignorebypass");
    }
    ...
    private void zkik(CommandSender sender) {
        ...
        Player[] players = getKickablePlayers(plugin);
        if (players.length < 1) {
    ...
    private void getzkik(CommandSender sender, int num) {
        ...
        if (isOnlinePlayer(senderName)) {
            Bukkit.getPlayer(senderName).getInventory().addItem(getZanakik(plugin, num<64?num:64));
        }
    }
    ...
    protected void kik(Player player) {
        player.getInventory().addItem(getZanakik(plugin, 1));

        Bukkit.getBanList(Type.NAME).addBan(player.getName(), "Zanakik", getZkikExpiresDate(), "zanakik");
