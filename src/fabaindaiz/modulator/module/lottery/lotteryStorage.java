...
import static fabaindaiz.modulator.module.lottery.lotteryUtil.*;
...
public class lotteryStorage implements Listener {
    ...
    ArrayList<Integer> winner = setWinner();
    String winnerCode = arrayToCode(winner);

    protected lotteryStorage(Modulator plugin, languageLoader lang) {
        this.usevault = plugin.getConfiguration().getConfig().getBoolean("lottery.usevault");
        this.novalid = plugin.getConfiguration().getConfig().getBoolean("lottery.novalid");
        this.plugin = plugin;
        this.lang = lang;
    }
    ...
    protected void drawWinner() {
        ...
        this.winnerCode = arrayToCode(this.winner);
    }
    ...
    protected void newLottery(CommandSender sender) {
        ...
        inventory.setItem(8, getWhiteGlass(lang, 0));
        inventory.setItem(17, getRedWool(lang.get("lottery.give3")));

        for (int i = 0; i < 7; i++) {
            inventory.setItem(i, getRedWool(String.valueOf(i + 1)));
        }
        for (int i = 7; i < 14; i++) {
            inventory.setItem(i + 2, getRedWool(String.valueOf(i + 1)));
        }
        ...
        if (selected <= 8) {
            if (slot < 7 || (slot >= 9 && slot <= 15)) {
                ...
                if (playerNumbers[number] == 0) {
                    ...
                    playerInv.setItem(slot, getGreenWool(String.valueOf(number + 1)));
                    playerInv.setItem(8, getWhiteGlass(lang, selected + 1));
                    numbers.get(senderName)[number] = 1;

                    if (selected == 7) {
                        playerInv.setItem(17, getGreenWool(lang.get("lottery.give3")));
                    }
                    return;
                } else {
                    playerInv.setItem(slot, getRedWool(String.valueOf(number + 1)));
                    playerInv.setItem(8, getWhiteGlass(lang, selected - 1));
                    numbers.get(senderName)[number] = 0;

                    if (selected == 8) {
                        playerInv.setItem(17, getRedWool(lang.get("lottery.give3")));
                    }
                    return;
                }
            }
    ...
    protected void giveTicket(Player player) {
        ...
        player.getInventory().addItem(getLotteryTicker(lang, senderName, lore));
    }
